...
public class MerchantRateController {
	...
	public ResultMessage merchant_rate_edit_ajax(MerchantRate merchantRate) {
		...
		if (merchantRate.getProductRate() == null || new BigDecimal(5).compareTo(merchantRate.getProductRate()) < 0) {
			return new ResultMessage(ResponseEnum.M4000.getCode(), "利率不能大于5%");
		}
		if (merchantRate.getOverdueRate() == null || new BigDecimal(8).compareTo(merchantRate.getOverdueRate()) < 0) {
			return new ResultMessage(ResponseEnum.M4000.getCode(), "逾期费率不能大于8%");
		}
